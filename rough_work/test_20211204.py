...
def objectiveFunction2(C,T,X,ftype='r'):
    n_tours = X.shape[0]
    ...
    if ftype=='td': #Total deviation from average
        for i,tour in enumerate(X):
            distance = np.sum(np.multiply(T,tour))
            total_distance += distance
...
def createChromosome(C,T,n_tours,ctype=1):
...
def generateInstance(n_cities,map_size):
    ...
    for i,city_i in enumerate(city_coordinates):
        for j,city_j in enumerate(city_coordinates):
            time_matrix[i,j] = distance_matrix[i,j]/rng.integers(low=20,high=91)
    return distance_matrix,city_coordinates,time_matrix
  
def createInitialPopulation(N,C,T,data,n_tours,ctype=1):
    population = []
    while len(population)<N:
        individual = createChromosome(C,T,n_tours,ctype)
...
def createOffspringPopulation(population,dist_matrix,T,tsub=2,selection_prob=0.9,
...
def main():
    ...
    instance_file_name = 'instances/eil51.txt'
    ...
    if instance_type=='random':
        ...
        C,data,T = generateInstance(n_cities,map_size)
        instance_file_name = 'random'
    else:
        print("Reading instance from file:",instance_file_name,"...")
        C,data,T = readInstance(instance_file_name)
    
    #cx_all = ['pmx','cycx','ox','hx']
    cx_all = ['hx']
    #mut_range = [0.01,0.05,0.08]
    mut_range = [0.05]
    exp_no = 0
    plt.figure()
    best_fronts = []
    ...
    for cx_type in cx_all:
        for mutation_probability in mut_range:
            for par_iter in range(10):
                ...
                population = createInitialPopulation(pop_size,C,T,data,n_tours,ctype)
                extra_front = []
                first_front = []
            ...
            final_front = nondominatedSort(best_fronts, 2)
            bestest_front = assignCrowdingDistance(final_front[0])
            bestest_front = sorted(bestest_front,key=functools.cmp_to_key(crowdedComparisonOperator))
            ...
            plt.plot(X,Y,'og')
            fronts_dict[exp_no] = {"cx":cx_type,"mu":mutation_probability,"first front":bestest_front}
            best_fronts.clear()
            exp_no += 1
    #plt.legend(["pmx,0.01","pmx,0.05","pmx,0.08","cx,0.01","cx,0.05","cx,0.08",
    #            "ox,0.01","ox,0.05","ox,0.08","hx,0.01","hx,0.05","hx,0.08"],
    #           loc="upper right", edgecolor="black")
    #plt.legend(["hx,0.05,pop=100","hx,0.05,pop=200","hx,0.05,pop=300"],
    #           loc="upper right", edgecolor="black")
    a = [525,550,575,600,625,650,675,700,725,750,760,777,800,820]
    b = [130,120,105,80,70,50,40,30,25,20,11,10,9,6]
    plt.plot(a,b,'ob')
    plt.xlim([450,900])
    plt.ylim([0,140])
    plt.plot(777,4.5,'xr')
    
    plt.savefig("eil51_m7.svg")
    plt.show()
    with open("Results/data/exp_1.pkl","wb") as wrfile:
        pickle.dump(bestest_front,wrfile)
